...
-- |
-- Module      : Cardano.MPFS.HTTP.RequestInsertFactsSpec
-- Description : Tests for POST /facts/request/insert
-- License     : Apache-2.0
module Cardano.MPFS.HTTP.RequestInsertFactsSpec (spec) where
...
spec :: Spec
spec = describe "POST /facts/request/insert" $ do
    it "is routed and rejects malformed addresses with 400" $ do
        ctx <- mkTestContext
        resp <- postJson ctx "/facts/request/insert" badRequest
            ...
            Right (Object swagger) ->
                case KM.lookup "paths" swagger of
                    Just (Object paths) -> do
                        KM.member "/facts/request/insert" paths
                            `shouldBe` True
                        KM.member "/tx/request/insert" paths
                            `shouldBe` False
                    _ ->
                        expectationFailure
    ...
    it "packages request-insert facts without unsigned tx cbor" $ do
        ...
        case toJSON facts of
            ...
            _ ->
                expectationFailure
                    "Expected RequestInsertFacts JSON object"

-- | Deliberately malformed serialized address with valid token/key/value
-- fields, so handler-level address validation determines the status.
badRequest :: String
badRequest =
    "{\"token\":\"63616665\",\"key\":\"6b\",\"value\":\"76\",\"address\":\"00\"}"
