...
-- |
-- Module      : Cardano.MPFS.HTTP.RequestDeleteFactsSpec
-- Description : Tests for POST /facts/request/delete
-- License     : Apache-2.0
module Cardano.MPFS.HTTP.RequestDeleteFactsSpec (spec) where
...
spec :: Spec
spec = describe "POST /facts/request/delete" $ do
    it "is routed and rejects malformed addresses with 400" $ do
        ctx <- mkTestContext
        resp <- postJson ctx "/facts/request/delete" badRequest
            ...
            Right (Object swagger) ->
                case KM.lookup "paths" swagger of
                    Just (Object paths) -> do
                        KM.member "/facts/request/delete" paths
                            `shouldBe` True
                        KM.member "/tx/request/delete" paths
                            `shouldBe` False
                    _ ->
                        expectationFailure
    ...
    it "packages request-delete facts without unsigned tx cbor" $ do
        ...
        case toJSON facts of
            ...
            _ ->
                expectationFailure
                    "Expected RequestDeleteFacts JSON object"
