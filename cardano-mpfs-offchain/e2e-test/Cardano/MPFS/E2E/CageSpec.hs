        ...
        $ \_sock _startMs cfg ctx -> do
            ...
            upd2Result <-
                submitTx
                    (submitter ctx)
                    signedUpd2
            assertSubmitted upd2Result
            awaitTx

            -- Mixed batch: insert "bbb", insert
            -- "ccc", then delete "bbb", then update
            unsignedIns2 <-
                requestInsert
                    (txBuilder ctx)
                    tokenId
            ...
            ins3Result <-
                submitTx
                    (submitter ctx)
                    signedIns3
            assertSubmitted ins3Result
            awaitTx

            -- First update to get bbb+ccc in trie
            unsignedUpd3 <-
                updateToken
                    (txBuilder ctx)
                    tokenId
            ...
            ins4Result <-
                submitTx
                    (submitter ctx)
                    signedIns4
            assertSubmitted ins4Result
            awaitTx

            -- Mixed batch update (delete + insert)
            unsignedUpd4 <-
                updateToken
                    (txBuilder ctx)
                    tokenId
            ...
            upd4Result <-
                submitTx
                    (submitter ctx)
                    signedUpd4
            assertSubmitted upd4Result
            awaitTx

            -- Final check: cage still has state UTxO
            finalUtxos <-
                queryUTxOs
                    (provider ctx)
                    scriptAddr
