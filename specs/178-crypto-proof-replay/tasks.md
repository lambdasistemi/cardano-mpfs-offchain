...
- [ ] T003 [P] Extend the `library` stanza of `/code/cardano-mpfs-offchain-issue-226/cardano-mpfs-client/cardano-mpfs-client.cabal` to add `build-depends: mts:csmt-verify`, `mts:mpf-write`, and (only if T013 needs it) `cborg`. Keep `base16-bytestring`, `aeson`, `base`, `bytestring`, `text` as-is. Audit: no `cardano-ledger-*`, no `crypton`, no `rocksdb*`, no new C-FFI dep. **Deferred to Phase 2** — `-Werror -Wunused-packages` requires the build-dep to land in the same commit as its first import site (`Verify/Replay.hs` in T012/T016). Re-correction: true cabal path is `cardano-mpfs-client/` at repo root (the `cardano-mpfs-offchain/cardano-mpfs-client/` path in the original task is a typo).
- [ ] T004 [P] Add the new exposed-module list to `cardano-mpfs-client.cabal`: `Cardano.MPFS.Client.Verify.Replay`, `Cardano.MPFS.Client.Verify.DSL`, `Cardano.MPFS.Client.Verify.Examples`. Extend `other-modules` under `test-suite unit-tests` to include `Cardano.MPFS.Client.VerifySpec` and `Cardano.MPFS.Client.Verify.DSLSpec`. **Deferred to Phase 2** (same reason as T003); modules are exposed alongside their first `.hs` file.
