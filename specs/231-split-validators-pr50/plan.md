...
| `CageScripts` (or equivalent in `OnChain.hs`) | `Cardano.MPFS.Core.OnChain` | Drop `Mint(..)` from exports + import. Update hardcoded `cageScriptHash` from PR #48's per-token hash to the current global state validator hash derived from the pinned blueprint. |
