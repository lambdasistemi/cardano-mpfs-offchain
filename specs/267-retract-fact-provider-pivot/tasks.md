...
- [x] T011 Fix the final-head `FactsMatrixSpec` boot row collateral
      ...
      The focused local rerun also surfaced the same collateral
      selection class in the end row, so `endCageTx` now uses the
      same largest-lovelace selection rule.
- [x] T012 Rerun the focused facts matrix selector for seed
