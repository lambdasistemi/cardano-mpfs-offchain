...
**Landing model: eight per-endpoint vertical slices.** Each slice is
a paired (cardano-mpfs-offchain PR + lambdasistemi/moog PR) landing
in lockstep. The offchain slice hard-swaps a single endpoint
(`POST /tx/{op}` → `POST /facts/{op}`) and the moog slice migrates
the matching call site. Slices are independent and carry their own
scaffolding — there is no shared foundation PR. Slice ordering is
recommended (S1 boot → S8 reject) but not constrained beyond the
S7/S8 tier-3 dependency on [#248](https://github.com/lambdasistemi/cardano-mpfs-offchain/issues/248).

Eight per-slice cutover windows replace the single big-bang cutover
in the original draft. Each window is typically same-day; the
production-deploy discipline (no commit inside any window) is
applied per slice. See `tasks.md` for the slice-by-slice breakdown.
...
└── tasks.md             # Per-endpoint slice breakdown — generated by /speckit.tasks
```
...
**Structure Decision**: Two repositories move together, **per
endpoint, eight times**. Each of the eight endpoints lands as one
paired (offchain, moog) slice. Within a slice:

1. **Offchain slice PR** hard-swaps `POST /tx/{op}` for `POST /facts/{op}`
   in a single commit: adds the new endpoint, removes the legacy
   endpoint, adds the cage helper + verifier in `cardano-mpfs-client`,
   adds any new indexer primitive, captures the legacy CBOR golden
   vector before deletion, regenerates swagger.
2. **Moog slice PR** adds the `MPFS.Facts.{op}` client function,
   migrates moog's call sites for that op, bumps the
   `cardano-mpfs-offchain` source-repository-package pin to the
   offchain slice's merge SHA.

Between the two merges, the **per-slice cutover window** is open
for that single op only: the server publishes `/facts/{op}` but no
longer `/tx/{op}`, and moog's binary still calls `/tx/{op}` until
its paired PR merges. Other endpoints are unaffected. The window
closes when the paired moog PR is on moog main.

Each slice is independent. No shared foundation PR — every slice
carries whatever modules (e.g., `WalletPolicy`, `BuildError`,
verifier-framework helpers) it needs the first time they appear.
The recommended landing order (S1 boot first, since it lands the
broadest shared scaffolding) keeps subsequent slices small but is
not a hard constraint.

A safer alternative — single coordinated multi-repo merge across
both repos — is rejected because the repositories don't share a CI
pipeline and a synchronised cross-repo merge is fragile. A single
big-bang cutover across all eight endpoints in one PR is rejected
because reviewers cannot independently land each endpoint, and a
multi-endpoint regression discovered post-merge would require
rolling back all eight. The per-endpoint slicing localises risk
per op; FR-011's lockstep constraint is satisfied by ensuring no
production deploy uses any per-slice cutover window.
...
- **Q0-6 — Verifier signature shape**: `verifyBootFacts`,
  ...
  be awkward).
- **Q0-7 — Cross-repo sequencing**: per-endpoint hard swap, eight
  slices. Each slice is one paired (cardano-mpfs-offchain PR +
  lambdasistemi/moog PR) landing in lockstep for one op only.
  The original single big-bang cutover model was rejected because
  reviewers couldn't land endpoints independently and a regression
  in any one would force rollback of all eight. Per-slice CI-gated
  windows replace the single window; production deploys must not
  use commits in any per-slice window.
...
producing code paths after the pivot.

The constitution amendment is **already on main** (bumped to 2.0.0
in [PR #259](https://github.com/lambdasistemi/cardano-mpfs-offchain/pull/259)).
Each of the eight per-endpoint slices ships under the active
2.0.0 constitution. The amended Principle IV's wording therefore
governs the entire pivot rollout from S1 onward, not the moment of
a single cutover commit.
...
| Eight per-slice two-repo lockstep landings (one per endpoint) | Hard-swap per endpoint requires the offchain endpoint removal and the moog call-site migration to land in the same release window for that op; CI gates each window | A single big-bang cutover across all eight endpoints in one (offchain, moog) pair is rejected — reviewers cannot land endpoints independently and a regression in any one endpoint forces rollback of all eight. Per-slice windows localise the lockstep risk to one op at a time |
