...
test("registers and ends a token through the real browser reactor on preprod", async ({
  page,
  request,
}) => {
  ...
  await expect.poll(() => submittedTxIds.length, { timeout: 60_000 }).toBe(1);
  ...
  await expect.poll(() => submittedTxIds.length, { timeout: 60_000 }).toBe(2);
    ...
    .poll(
      async () => {
        ...
        for (const token of tokens) {
          ...
          if (state?.state?.state?.owner === preprodRequesterOwner) {
            matched = token;
            return token;
          }
        }
