...
let
  ...
  renderFlags = lib.concatStringsSep "" (lib.mapAttrsToList (pkg: flags: ''
    package ${pkg}
      flags: ${flags}
  '') forks.packageFlags);

  renderGhcOptions = lib.concatStringsSep "" (lib.mapAttrsToList (pkg: opts: ''
    package ${pkg}
      ghc-options: ${opts}
  '') forks.packageGhcOptions);

  renderConstraints = if forks.constraints == [ ] then
    ""
  else
    "constraints: " + lib.concatStringsSep ", " forks.constraints + "\n";

  renderAllowNewer = if forks.allowNewer == [ ] then
    ""
  else
    "allow-newer: " + lib.concatStringsSep ", " forks.allowNewer + "\n";
